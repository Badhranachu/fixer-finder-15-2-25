  ...
  try {
    ...
    if (product.quantity < quantity) {
      ...
      return res.render('user/confirmed', {
        errorMessage: 'Technician is busy, not enough slots available',
    ...
    if (updatedProduct.quantity === 0) {
      ...
      // Set a timer to restore the quantity after 1 minute
      setTimeout(async () => {
        await db.get()
          .collection(collection.PRODUCT_COLLECTION)
          .updateOne(
        ...
        console.log(`Product ID ${productId} quantity restored to 5 after 1 minute.`);
      }, 86400000); // 
    }
...
router.get('/orders', async (req, res) => {
  ...
  try {
    ...
    if (!orders || orders.length === 0) {
      ...
      return res.render('user/orders', { orders: [], user: req.session.user });
    }

    for (const order of orders) {
      const product = await productHelpers.getProductById(order.productId);
      console.log('Product for order:', product);
    ...
    res.status(500).render('user/orders', { orders: [], user: req.session.user });
  }
});
...
router.get('/category', async (req, res) => {
  const selectedCategory = req.query.category;  // Get category from query
  ...
  try {
    ...
    if (!selectedCategory) {
      ...
      products = await db.get().collection(collection.PRODUCT_COLLECTION).find({ category: selectedCategory }).toArray();
    }
...
router.get('/getcategories', async (req, res) => {
  try {
    const categories = await db.get().collection(collection.CATEGORY_COLLECTION).find().toArray();
    console.log("Categories fetched from DB:", categories);
    res.json({ success: true, categories });  // ✅ Send JSON response
  } catch (error) {
...
router.post('/category', async (req, res) => {
  ...
  try {
    // Fetch the category from the database that matches the passed category
    const categoryData = await db.get().collection(collection.CATEGORY_COLLECTION).findOne({ name: category });
    ...
    // Send the matched products to the Handlebars template
    res.render('user/category', {
      success: true,
      categoryData,  // Pass the category data to the template
      products,      // Pass the products to the template
      user: req.session.user // Optionally pass the user session if needed
    });
    ...
    res.status(500).json({ success: false, message: "Failed to fetch category or products" });
  }
});
...
router.get('/admin-products', async (req, res) => {
  if (!req.session.admin) {
      return res.redirect('/admin-login'); // ✅ Redirect to login if no admin is in session
  }
  ...
  try {
      ...
      // Render view-products page with admin session and product list
...
router.get('/admin-login', (req, res) => {
  res.render('admin/admin-login'); // ✅ Renders login page if no admin is found
});
...
router.get('/admin-logout', (req, res) => {
  req.session.admin = null; // ✅ Destroy admin session
